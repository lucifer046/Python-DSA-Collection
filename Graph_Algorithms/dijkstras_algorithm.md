...
```
  ...
  Adjacency List:
  Node 0: → 1(cost 4), → 2(cost 2)
  Node 1: → 0(cost 4), → 2(cost 1), → 3(cost 5)
  Node 2: → 0(cost 2), → 1(cost 1), → 3(cost 8), → 4(cost 10)
  Node 3: → 1(cost 5), → 2(cost 8), → 4(cost 2)
  Node 4: → 2(cost 10), → 3(cost 2)
```
...
```
  ...
  AFTER relaxation:
  Distance to node 1 = 3  (via path: 0 → 2 → 1)  ← CHEAPER! Update! [done]
```
...
```
  ...
  All costs positive ✅        Has negative cost [X]
